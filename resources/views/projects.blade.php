            ...
            <div class="flex w-full items-center justify-end gap-2">
                ...
                <div>
                    <form action="{{ route('projects.search') }}" method="GET" class="flex gap-2">
                        {{-- input --}}
                        <input name="search" type="text" placeholder="プロジェクトを検索" value="{{ request('search') }}" class="flex h-9 w-80 rounded-sm border-0 bg-neutral-400 px-3 py-2 text-sm transition-all hover:bg-neutral-300 focus:bg-neutral-300" />

                        {{-- sort --}}
                        <select name="sort" class="flex h-9 w-48 items-center justify-center rounded-sm border-0 bg-white px-4 py-0 text-sm font-medium text-neutral-500 shadow-sm transition-colors duration-200">
                ...
                <div>
                    <a href="{{ route('projects.create') }}" type="button"
                        class="focus:shadow-outline inline-flex items-center justify-center rounded-sm bg-neutral-800 px-4 py-2 text-sm font-medium tracking-wide text-white transition-colors duration-200 hover:bg-neutral-900 focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:ring-offset-2">
                        プロジェクトを新規作成
                    </a>
                </div>
            </div>
            ...
            <div class="rounded border border-neutral-200 bg-white p-6">
                ...
                @foreach ($projects as $key => $project)
                    <div class="mb-5 border-b border-neutral-200">
                        ...
                        <div class="border-0 border-neutral-200 pb-4">
                            <div class="flex flex-row items-start justify-between gap-2">

                                {{-- id --}}
                                <div class="text-md font-bold text-neutral-400">
                                    {{ $project->id }}
                                </div>

                                {{-- publish --}}
                                <div class="flex items-center justify-start gap-2 border-0 border-neutral-200">
                                    @if ($project->status)
                                        {{-- 公開中 --}}
                                ...
                                <div class="ml-auto flex flex-row gap-2">
                                    ...
                                    <div class="flex flex-row gap-1 text-sm">
                                        <button id="toggleButton{{ $key }}" class="flex flex-row items-center gap-2 rounded border border-neutral-200 px-3 py-1 text-sm hover:bg-neutral-100" type="button">
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <a class="flex h-8 items-center justify-center rounded border border-neutral-200 px-4 text-sm hover:bg-neutral-100" href="{{ route('postuser.index', ['uuid' => $project->uuid]) }}" target="_blank">投稿ページを開く</a>
                                    </div>

                                    {{-- show --}}
                                    <a href="{{ route('projects.show', ['id' => $project->id]) }}" @click="menuBarOpen=false" class="flex h-8 items-center justify-center rounded border border-neutral-200 px-4 text-sm hover:bg-neutral-100">
                                        <span>詳細</span>
                                    </a>

                                    {{-- edit --}}
                                    <a href="{{ route('projects.edit', $project->id) }} @click=" menuBarOpen=false" class="flex h-8 items-center justify-center rounded border border-neutral-200 px-4 text-sm hover:bg-neutral-100">
                                        <span>編集</span>
                                    </a>
                                    ...
                                    <form action="{{ route('projects.duplicate', $project->id) }}" method="POST">
                                        @csrf
                                        <button type="submit" @click="menuBarOpen=false" class="flex h-8 items-center justify-center rounded border border-neutral-200 px-4 text-sm hover:bg-neutral-100">
                                            <span>複製</span>
                                        </button>
                                    </form>
                        ...
                        {{-- forms --}}
                        <div id="forms{{ $key }}" class="hidden border-0 border-neutral-200 px-4 pb-2">
