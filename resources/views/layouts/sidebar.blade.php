<aside class="h-svh flex w-64 shrink-0 flex-col overflow-y-scroll border-r border-neutral-300 bg-white px-3 py-6">
    ...
    <div class="mb-8 ml-4 flex shrink-0 items-center text-xl font-extrabold">
        <a href="{{ route('dashboard') }}">
            <p class="block h-9 w-auto fill-current text-gray-800">Proofsheet</p>
        </a>
    </div>
    ...
    <nav class="mb-8 mt-auto">
        <div class="flex flex-col">
            ...
            <a class="rounded-md px-4 py-1.5 text-sm text-gray-500 transition hover:bg-gray-100" href="{{ route('profile.edit') }}">アカウント</a>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button class="block w-full rounded-md px-4 py-1.5 text-left text-sm text-red-500 transition hover:bg-red-100" type="submit">ログアウト</button>
            </form>
        </div>
    </nav>
